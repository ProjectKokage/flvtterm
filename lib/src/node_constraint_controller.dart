...
final class VrmNodeConstraintController {
  ...
  VrmNodeConstraintController(this.model)
    : _plan = _buildConstraintPlan(model.gltf.nodes);
...
List<_RunnableNodeConstraint> _buildConstraintPlan(List<GltfNode> nodes) {
  final parents = _buildConstraintParentMap(nodes);
...
Map<int, int> _buildConstraintParentMap(List<GltfNode> nodes) {
  ...
  return Map.unmodifiable(parents);
}
